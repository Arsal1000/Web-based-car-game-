      ...
      class Audio {
        ...
        load(src, key, callback) {//loads an audio file from a given source and associates it with a specified key
          let _self = this;
      ...
      //defines required global varriables
      const highscores = [];
      const width = 800;
      ...
      // loop
      let then = timestamp();
      const targetFrameRate = 1000 / 25; //sets the framerate (25fps)
      ...
      function update(step) {
        ...
        pos += speed;
        while (pos >= N * segL) pos -= N * segL;//makes sure that car stays inside the track
        while (pos < 0) pos += N * segL;//if it does it wraps around
